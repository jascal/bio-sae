# architecture bio_sae_attn_topk_f1

> Attention-prefixed Top-k sparse autoencoder (k=32, n_heads=4, attn_dropout=0.0). A MultiHeadAttention block (with residual + LayerNorm) precedes the SAE encoder so each position can see cross-sequence context before being encoded. Sparsity is enforced by TopK; no L1 / L0 penalty needed.
